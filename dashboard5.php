...
// Define the number of question5 per page
$question5_per_page = 12;

// Get the current page number
$current_page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1;

// Calculate the offset for the LIMIT clause in the SQL query
$offset = ($current_page - 1) * $question5_per_page;

// Modify the SQL query to include the LIMIT clause
if(isset($_GET['q'])) {
    $search = $_GET['q'];
    $result = mysqli_query($conn, "SELECT question5.id, question5.question, user_answers.answer FROM question5 LEFT JOIN user_answers ON question5.id=user_answers.question_id WHERE question LIKE '%$search%' LIMIT $offset, $question5_per_page");
} else {
    $result = mysqli_query($conn, "SELECT question5.id, question5.question, user_answers.answer FROM question5 LEFT JOIN user_answers ON question5.id=user_answers.question_id LIMIT $offset, $question5_per_page");
}
// Count the total number of question5 in the database
$count_query = mysqli_query($conn, "SELECT COUNT(*) as total_question5 FROM question5");
$count_result = mysqli_fetch_assoc($count_query);
$total_question5 = $count_result['total_question5'];

// Calculate the total number of pages
$total_pages = ceil($total_question5 / $question5_per_page);
