...
// Define the number of question2 per page
$question2_per_page = 12;

// Get the current page number
$current_page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1;

// Calculate the offset for the LIMIT clause in the SQL query
$offset = ($current_page - 1) * $question2_per_page;

// Modify the SQL query to include the LIMIT clause
if(isset($_GET['q'])) {
    $search = $_GET['q'];
    $result = mysqli_query($conn, "SELECT question2.id, question2.question, user_answers.answer FROM question2 LEFT JOIN user_answers ON question2.id=user_answers.question_id WHERE question LIKE '%$search%' LIMIT $offset, $question2_per_page");
} else {
    $result = mysqli_query($conn, "SELECT question2.id, question2.question, user_answers.answer FROM question2 LEFT JOIN user_answers ON question2.id=user_answers.question_id LIMIT $offset, $question2_per_page");
}
// Count the total number of question2 in the database
$count_query = mysqli_query($conn, "SELECT COUNT(*) as total_question2 FROM question2");
$count_result = mysqli_fetch_assoc($count_query);
$total_question2 = $count_result['total_question2'];

// Calculate the total number of pages
$total_pages = ceil($total_question2 / $question2_per_page);
