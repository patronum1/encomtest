...
// Define the number of question7 per page
$question7_per_page = 12;

// Get the current page number
$current_page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1;

// Calculate the offset for the LIMIT clause in the SQL query
$offset = ($current_page - 1) * $question7_per_page;

// Modify the SQL query to include the LIMIT clause
if(isset($_GET['q'])) {
    $search = $_GET['q'];
    $result = mysqli_query($conn, "SELECT question7.id, question7.question, user_answers.answer FROM question7 LEFT JOIN user_answers ON question7.id=user_answers.question_id WHERE question LIKE '%$search%' LIMIT $offset, $question7_per_page");
} else {
    $result = mysqli_query($conn, "SELECT question7.id, question7.question, user_answers.answer FROM question7 LEFT JOIN user_answers ON question7.id=user_answers.question_id LIMIT $offset, $question7_per_page");
}
// Count the total number of question7 in the database
$count_query = mysqli_query($conn, "SELECT COUNT(*) as total_question7 FROM question7");
$count_result = mysqli_fetch_assoc($count_query);
$total_question7 = $count_result['total_question7'];

// Calculate the total number of pages
$total_pages = ceil($total_question7 / $question7_per_page);
